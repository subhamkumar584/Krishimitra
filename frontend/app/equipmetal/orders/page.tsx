...
import { getCurrentUser, getOrders, approveOrder, rejectOrder } from "../../../lib/api";
import { Loader2, ArrowLeft, Truck, X } from "lucide-react";

interface User {
  id: number;
  role: "equipmetal" | "admin" | string;
  name: string;
  email: string;
}
...
export default function EquipmetalOrdersPage() {
  ...
  useEffect(() => {
    if (!loadingUser && user && (user.role === "equipmetal" || user.role === "admin")) {
      fetchOrders();
    }
  }, [loadingUser, user]);

  const canApprove = (o: OrderSummary) => o.payment_status === "CAPTURED" && !["SHIPPED","DELIVERED","CANCELLED"].includes(o.status);
    ...
    return (
      <div className="min-h-screen bg-[color:var(--background)] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-[color:var(--foreground)]/70 mb-4">Only equipmetal providers and admins can view seller orders.</p>
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center space-x-3">
            <Link href="/marketplace" className="px-3 py-2 border border-[color:var(--border)] rounded hover:bg-[color:var(--muted)]"><ArrowLeft className="w-4 h-4" /> Back</Link>
            <h1 className="text-3xl font-bold text-[color:var(--foreground)]">Seller Orders</h1>
          </div>
          <button onClick={fetchOrders} className="text-sm px-3 py-2 rounded-md border border-[color:var(--border)] hover:bg-[color:var(--muted)]">Refresh</button>
        </div>
        ...
        {!loading && !error && orders.length === 0 && (
          <div className="text-center py-16 border border-[color:var(--border)] rounded-lg bg-[color:var(--card)]">
            ...
            <p className="text-[color:var(--foreground)]/60">Paid orders from buyers will appear here.</p>
          </div>
        )}
              ...
              <div key={o.id} className="bg-[color:var(--card)] border border-[color:var(--border)] rounded-xl p-5">
                ...
                <div className="mt-4 flex items-center justify-end gap-2">
                  <button
                    ...
                    disabled={busy === o.id}
                    className="inline-flex items-center px-4 py-2 rounded-md border border-[color:var(--border)] text-red-400 hover:bg-red-900/20"
                  >
                    <X className="w-4 h-4 mr-2" /> Reject
                  </button>
                  <button
                    onClick={() => onApprove(o.id)}
                    disabled={!canApprove(o) || busy === o.id}
